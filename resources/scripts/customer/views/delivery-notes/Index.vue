<template>
  <BasePage>
    <BasePageHeader :title="'Albaranes'">
      <BaseBreadcrumb>
        <BaseBreadcrumbItem :title="$t('general.home')" :to="`/${globalStore.companySlug}/customer/dashboard`" />
        <BaseBreadcrumbItem :title="'Albaranes'" to="#" active />
      </BaseBreadcrumb>
    </BasePageHeader>
    <BaseEmptyPlaceholder v-if="showEmptyScreen" :title="'No hay albaranes'" :description="'Aquí aparecerán tus albaranes.'">
    ...
    <div v-show="!showEmptyScreen" class="relative table-container">
      <BaseTable ref="table" :data="fetchData" :columns="itemColumns" :placeholder-count="5" class="mt-10">
        ...
        <template #cell-number="{ row }">
          <router-link :to="{ path: `delivery-notes/${row.data.id}/view` }" class="font-medium text-primary-500">
            {{ row.data.delivery_note_number || row.data.invoice_number }}
          </router-link>
        </template>
...
const showEmptyScreen = computed(() => !deliveryNoteStore.totalDeliveryNotes && !isFetchingInitialData.value)
async function fetchData({ page, sort }) {
  isFetchingInitialData.value = true
  let response = await deliveryNoteStore.fetchDeliveryNotes({ orderByField: sort.fieldName || 'created_at', orderBy: sort.order || 'desc', page }, globalStore.companySlug)
