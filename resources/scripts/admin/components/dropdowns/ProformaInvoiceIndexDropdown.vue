  ...
  <BaseDropdown>
    ...
    >
      ...
      <CompartirPdf
        ...
        :url="`/proforma-invoices/pdf/${row.unique_hash}`"
        :nombre="row.proforma_invoice_number || (row.id ? `#${row.id}` : 'documento')"
      />
...
function abrirPdf(row) {
  if (row?.unique_hash) {
    window.open(`/proforma-invoices/pdf/${row.unique_hash}`, '_blank')
  }
}
